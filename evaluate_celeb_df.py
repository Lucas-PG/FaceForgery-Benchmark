...
def main(argv=None):
    ...
    return research_main(['evaluate-legacy','--checkpoint',str(args.checkpoint),
                          '--manifest',str(args.manifest_csv),'--root',str(args.crops_dir),
                          '--calibration',str(args.calibration),'--output',str(args.output_dir),
                          '--class-one',args.class_one,'--device',args.device,
                          '--batch-size',str(args.batch_size),'--workers',str(args.num_workers)])
