...
@pytest.mark.parametrize('family',['resnet','mobilenet','xception','dino','clip','vit'])
@pytest.mark.parametrize('regime,expected',[('scratch',False),('scratch_robust',False),('finetune',True),('finetune_robust',True)])
def test_robust_suffix_does_not_change_new_initialization(family,regime,expected):
    assert uses_pretraining(SimpleNamespace(regime=regime,initialization_contract='regime-v2'),family) is expected
...
def test_unknown_regime_rejected():
    with pytest.raises(ValueError): uses_pretraining(SimpleNamespace(regime='fine_tunish'),'resnet')
...
def test_saved_config_preserves_legacy_build_identity(tmp_path):
    ...
    root=tmp_path/'run'; (root/'results').mkdir(parents=True)
    (root/'results/run_config.json').write_text(json.dumps({'model_family':'xception','regime':'finetune_robust','fourier_mode':'none'}))
    run=TrainedRun('xception','none','finetune_robust',42,root,root/'best.pth',{})
