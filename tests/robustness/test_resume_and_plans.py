...
def test_completed_epoch_resume_matches_uninterrupted_weights(tmp_path, monkeypatch):
    ...
    baseline=copy.deepcopy(cfg); baseline['output_root']=str(tmp_path/'uninterrupted')
    baseline_dir=Path(plan(baseline)['run_dir'])
    engine.train(baseline)
    a=torch.load(run_dir/'last.pt',map_location='cpu',weights_only=True)['state_dict']
    b=torch.load(baseline_dir/'last.pt',map_location='cpu',weights_only=True)['state_dict']
    assert a.keys()==b.keys()
    assert all(torch.equal(a[key],b[key]) for key in a)
    measured=profile(Namespace(run=run_dir,output=tmp_path/'timing.json',device='cpu',batch_size=1,warmup=1,iterations=3))
...
def test_ablation_expansion_keeps_nonexperimental_settings(tmp_path):
    ...
    result=expand(Namespace(template=template,plan='configs/research/ablation-plan.json',
                           output=tmp_path/'plan',variants=['rgb_basic','rgb_robust'],seeds=[42,123]))
...
def test_seed_identity_excludes_only_seed_and_nonscientific_paths(tmp_path):
    ...
    cfg=read_config(config_file(tmp_path,paths))
    a=plan(cfg)
    other=copy.deepcopy(cfg); other['training']['seed']=123; other['output_root']='elsewhere'
    b=plan(other)
    assert evaluation_identity(a)['condition_sha256']==evaluation_identity(b)['condition_sha256']
    other['training']['lr']*=2
    c=plan(other)
    assert evaluation_identity(a)['condition_sha256']!=evaluation_identity(c)['condition_sha256']
...
def test_model_parameters_rejected_during_planning(tmp_path):
    ...
    cfg=yaml.safe_load(file.read_text()); cfg['model']['width']=7
    file.write_text(yaml.safe_dump(cfg))
    with pytest.raises(ValueError,match='width'): read_config(file)
