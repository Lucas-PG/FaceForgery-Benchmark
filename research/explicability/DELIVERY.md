...
Evidence: [successful contract run](https://github.com/Lucas-PG/FaceForgery-Benchmark/actions/runs/34619443119), [final corrected-manuscript build](https://github.com/Lucas-PG/FaceForgery-Benchmark/actions/runs/34621541196). The paper packaging workflow checks the source/build identity, verifies the review header, rejects publication-status boilerplate, and preserves the original manuscript. Later source changes require their own checks; workflow evidence is tied to its actual commit, not an unqualified assertion about all future revisions.
