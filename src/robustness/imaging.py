...
MEAN = (.485,.456,.406)
STD = (.229,.224,.225)
MODES = {"none":3,"magnitude":1,"phase":1,"complex":2,"concat":4,"frequency_3":1,"concat_frequency":7}
RECIPE = {"name":"robust_v1", "jpeg_probability":.5,"jpeg_quality":[30,90],
          "blur_probability":.3,"blur_radius":[.5,2.],"noise_probability":.5,
          "noise_std":[.01,.06],"brightness_probability":.5,"brightness_factor":[.7,1.3]}
...
def to_tensor(image: Image.Image) -> torch.Tensor:
    return torch.from_numpy(np.array(image.convert("RGB"),dtype=np.float32).copy()).permute(2,0,1)/255.
...
def encode_tensor(raw: torch.Tensor, mode: str, in_channels: int | None = None) -> torch.Tensor:
    ...
    if mode=="concat_frequency" and in_channels==6:
        ...
        raise ValueError(f"Representation has {out.shape[1]} channels, checkpoint expects {in_channels}")
    if not torch.isfinite(out).all():
        raise ValueError("Nonfinite encoded input")
    return out[0] if single else out


def corrupt(image: Image.Image, rng: random.Random, generator: torch.Generator) -> torch.Tensor:
    """The named robust_v1 recipe is fixed independently of training seed."""
    if rng.random()<RECIPE['brightness_probability']:
        image=ImageEnhance.Brightness(image).enhance(rng.uniform(*RECIPE['brightness_factor']))
    if rng.random()<RECIPE['blur_probability']:
        image=image.filter(ImageFilter.GaussianBlur(rng.uniform(*RECIPE['blur_radius'])))
    ...
    if rng.random()<RECIPE['noise_probability']:
        noise=torch.randn(tensor.shape,generator=generator)*rng.uniform(*RECIPE['noise_std'])
        tensor=(tensor+noise).clamp(0,1)
    return tensor


class CanonicalDataset(Dataset):
    def __init__(self, frame, root, image_size: int, *, recipe: str="none", seed: int=42,
                 training: bool=False, hash_images: bool=False):
        ...
        if not training and recipe!="none":
            raise ValueError("Evaluation must use a separately materialized, frozen corruption manifest")
    ...
    def __getitem__(self, index):
        ...
        with Image.open(path) as opened:
            image=opened.convert("RGB").resize((self.size,self.size),Image.Resampling.BILINEAR)
        if self.training and self.recipe!="none":
            rng=random.Random(sample_seed(self.seed,self.epoch,row.sample_id,"geometry"))
