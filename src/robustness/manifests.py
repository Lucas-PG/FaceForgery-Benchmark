...
import numpy as np
import pandas as pd
from .provenance import SCHEMA, contained, digest, digest_file, relative_path, write_csv, write_json

REQUIRED = {"sample_id", "img_name", "label", "group_id", "dataset", "split"}
STRING_COLUMNS = ["sample_id", "img_name", "group_id", "video_id", "dataset", "split", "generator", "source_domain", "source_id", "sha256"]
...
def labels(values, convention: str) -> np.ndarray:
    if convention not in {"fake-is-1", "real-is-1"}:
        raise ValueError("Declare fake-is-1 or real-is-1; automatic inversion is forbidden")
    y = binary(values)
    return y if convention == "fake-is-1" else 1-y


def read_csv(path: str | Path) -> pd.DataFrame:
    return pd.read_csv(path, dtype={k: "string" for k in STRING_COLUMNS}, keep_default_na=False)
...
def validate(frame: pd.DataFrame, *, require_both: bool = True) -> pd.DataFrame:
    ...
    if frame.dataset.nunique() != 1 or frame.split.nunique() != 1:
        raise ValueError("One dataset and split per manifest; combine through an explicit protocol")
...
def save_manifest(frame: pd.DataFrame, output: str | Path, metadata: dict) -> dict:
    ...
    record = {**metadata, "schema": SCHEMA, "label_convention": "fake-is-1",
              "manifest_sha256": digest_file(output), "rows": len(frame),
              "dataset": frame.dataset.iloc[0], "split": frame.split.iloc[0],
...
def load_manifest(path: str | Path, *, require_both: bool = True):
    ...
    if record.get("schema") != SCHEMA or record.get("label_convention") != "fake-is-1":
        raise ValueError("Uncertified label convention; explicitly convert the original manifest")
    ...
    if len(frame) != record["rows"] or frame.dataset.iloc[0] != record["dataset"] or frame.split.iloc[0] != record["split"]:
        raise ValueError("Manifest identity differs from certificate")
    return frame, record


def convert_manifest(source: str | Path, output: str | Path, *, dataset: str,
                     split: str, label_column: str, convention: str,
    ...
    result = pd.DataFrame({"img_name": names, "label": labels(old[label_column], convention)})
    ...
    if group_column:
        if group_column not in old or old[group_column].isna().any() or old[group_column].astype(str).str.strip().eq("").any():
    ...
    return save_manifest(result[ordered], output, {
        "source_sha256": digest_file(source), "source_label_column": label_column,
        "source_label_convention": convention, "group_column": group_column,
        "group_unit": group_column or "image-only (identity/source dependence unresolved)",
...
def assert_disjoint(train: pd.DataFrame, val: pd.DataFrame) -> dict:
    ...
    if set(train.split) != {"train"} or set(val.split) != {"val"}:
        raise ValueError("Training requires split=train and selection requires source split=val")
    checks = {}
    for key in ("sample_id", "group_id", "source_id", "sha256"):
        ...
        overlap = a & b
        if overlap:
            raise ValueError(f"Train/validation overlap in {key}: {len(overlap)} values")
        checks[key] = "no overlap among provided values"
    return checks


def audit_images(manifest: str | Path, root: str | Path, *, output: str | Path,
                 hash_images: bool = False) -> dict:
    ...
    for row in frame.itertuples():
        try:
            ...
            if checksum and "sha256" in frame and str(row.sha256) and str(row.sha256) != checksum:
                raise ValueError("Image hash differs from manifest")
            if checksum:
                hashes.append({"sample_id": row.sample_id, "sha256": checksum})
        except Exception as error:
            errors.append({"sample_id": row.sample_id, "error": f"{type(error).__name__}: {error}"})
    result = {"manifest_sha256": certificate["manifest_sha256"], "expected": len(frame),
              "valid": len(frame)-len(errors), "errors": errors,
              "image_hashes": hashes, "complete": not errors,
              "duplicate_content_groups": []}
    if hashes:
        table = pd.DataFrame(hashes)
        result["duplicate_content_groups"] = [g.sample_id.tolist() for _,g in table.groupby("sha256") if len(g)>1]
