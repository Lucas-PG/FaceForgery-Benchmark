...
import numpy as np
import pandas as pd
from sklearn.metrics import roc_auc_score, average_precision_score, confusion_matrix, balanced_accuracy_score, brier_score_loss, log_loss
...
def probabilities(values) -> np.ndarray:
    ...
    if not np.isfinite(p).all() or ((p < 0) | (p > 1)).any():
        raise ValueError("Probabilities must be finite and lie in [0,1]; no sanitization")
    return p
...
def align(reference: pd.DataFrame, other: pd.DataFrame):
    ...
    if not a.sample_id.equals(b.sample_id):
        raise ValueError("Prediction populations differ; missing rows are not silently dropped")
...
def choose_threshold(y, p) -> float:
    ...
    ba = .5*((y.sum()-fn)/y.sum()+tn/(len(y)-y.sum()))
    return float(candidates[np.flatnonzero(ba == ba.max())[0]])


def summary(y, p, threshold: float = .5) -> dict:
    y, p = binary(y), probabilities(p)
    if len(y) != len(p) or not len(y) or not np.isfinite(threshold) or not 0 <= threshold <= np.nextafter(1.,2.):
        raise ValueError("Invalid metric inputs")
    pred = (p >= threshold).astype(int)
    tn, fp, fn, tp = confusion_matrix(y,pred,labels=[0,1]).ravel()
    both = len(np.unique(y)) == 2
    bins = np.minimum((p*10).astype(int),9)
    ece = sum(float(np.mean(bins==b))*abs(float(y[bins==b].mean())-float(p[bins==b].mean())) for b in range(10) if (bins==b).any())
    return {"n": len(y), "n_real": int((y==0).sum()), "n_fake": int((y==1).sum()),
            ...
            "accuracy": float((pred==y).mean()), "brier": float(brier_score_loss(y,p)),
            "log_loss": float(log_loss(y,p,labels=[0,1])), "ece_10_equal_width": ece,
...
def grouped_auc_interval(frame: pd.DataFrame, other: pd.DataFrame | None = None,
                         *, draws: int = 1000, seed: int = 42, confidence: float = .95) -> dict:
    ...
    for _ in range(draws):
        ...
        values.append(value)
    if len(values) < .8*draws:
        raise ValueError("Too many one-class bootstrap draws; report a different justified grouping/design")
    alpha = (1-confidence)/2
    lo,hi = np.quantile(values,[alpha,1-alpha])
    point = float(roc_auc_score(y,p))
    if q is not None:
        point = float(roc_auc_score(y,q))-point
    return {"estimate": point, "lower": float(lo), "upper": float(hi), "confidence": confidence,
            "requested_draws": draws, "valid_draws": len(values), "groups": len(groups), "seed":seed,
...
def complementarity(reference: pd.DataFrame, other: pd.DataFrame, *, reference_threshold: float,
                    other_threshold: float) -> tuple[dict,pd.DataFrame]:
    a,b = align(reference,other)
    # Validate threshold ranges using the same metric contract.
    summary(a.label,a.p_fake,reference_threshold); summary(b.label,b.p_fake,other_threshold)
    ea = (a.p_fake.to_numpy()>=reference_threshold) != a.label.to_numpy()
    eb = (b.p_fake.to_numpy()>=other_threshold) != b.label.to_numpy()
    groups = np.select([ea & ~eb, ~ea & eb, ea & eb], ["repaired_by_other","regressed_by_other","both_wrong"], default="both_correct")
    cases = a[["sample_id","group_id","label"]].copy()
    cases["category"] = groups
    counts = {k:int((groups==k).sum()) for k in ("repaired_by_other","regressed_by_other","both_wrong","both_correct")}
    return {"n":len(a), **counts, "reference_threshold":reference_threshold, "other_threshold":other_threshold,
            "other_correct_given_reference_wrong":float((ea & ~eb).sum()/ea.sum()) if ea.sum() else None,
            "oracle_accuracy_upper_bound_not_deployable":float(1-(ea & eb).mean())}, cases


def aggregate_videos(frame: pd.DataFrame) -> pd.DataFrame:
    frame = checked_predictions(frame)
    if "video_id" not in frame or frame.video_id.isna().any() or frame.video_id.astype(str).str.strip().eq("").any():
        raise ValueError("Complete video IDs are required")
    if frame.groupby("video_id").label.nunique().gt(1).any() or frame.groupby("video_id").group_id.nunique().gt(1).any():
        raise ValueError("Conflicting labels or sampling groups within a video")
    result = frame.groupby("video_id",sort=True).agg(label=("label","first"),group_id=("group_id","first"),
                                                    p_fake=("p_fake","mean"),n_frames=("sample_id","size")).reset_index()
...
def generator_metrics(frame: pd.DataFrame, threshold: float) -> dict:
    ...
    for c in ("generator","source_domain"):
        if frame[c].isna().any() or frame[c].astype(str).str.strip().isin(["","unknown"]).any():
            raise ValueError(f"Incomplete {c} annotations")
    rows = []
    for (domain,generator), fake in frame.loc[frame.label.eq(1)].groupby(["source_domain","generator"],sort=True):
        real = frame.loc[frame.label.eq(0) & frame.source_domain.eq(domain)]
        pair = pd.concat([real,fake])
        rows.append({"source_domain":str(domain),"generator":str(generator),
                     "real_reference":"all real rows in same source_domain", **summary(pair.label,pair.p_fake,threshold)})
    valid = [r["auc"] for r in rows if r["auc"] is not None]
    return {"groups":rows,"macro_auc":float(np.mean(valid)) if valid else None,
            "worst_group_auc":float(min(valid)) if valid else None,
            "auc_defined_groups":len(valid),"total_groups":len(rows),
