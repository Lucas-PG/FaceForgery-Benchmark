...
def predict(model, frame, root, *, image_size:int, device='cpu', batch_size=32,
            workers=0, mode=None, in_channels=None, positive_class='fake', hash_images=True):
...
def calibrate(predictions, *, manifest_record, output, model_sha256, checkpoint_class1='fake'):
    ...
    record={'schema':SCHEMA,'model_sha256':model_sha256,'source_manifest_sha256':manifest_record['manifest_sha256'],
            'source_dataset':manifest_record['dataset'],'selection_split':'val','label_convention':'fake-is-1',
            'checkpoint_class1':checkpoint_class1,'frame_threshold':choose_threshold(p.label,p.p_fake),
...
def evaluate(model, manifest, root, output, *, checkpoint_path, calibration_path,
             image_size, mode=None, in_channels=None, positive_class='fake', research_run=None, **kwargs):
    output=Path(output)
    ...
    try:
        ...
        report.update(schema=SCHEMA,manifest=record,checkpoint_sha256=checksum,
                      ...
                      preprocessing=(PREPROCESSING if mode is None else 'original-ImageDataset-NumPy-FFT-helpers'),
                      representation=mode or 'pilot-internal',software=source_identity())
        if research_run is not None: report['research_run']=research_run
        save_predictions(output/'predictions.csv',p,manifest_record=record,model_sha256=checksum,
                         checkpoint_class1=positive_class,metadata={'origin':'fresh image inference','preprocessing':report['preprocessing']})
        files=['predictions.csv','predictions.csv.json','metrics.json']
        if 'video' in report:
            write_csv(output/'video_predictions.csv',aggregate_videos(p)); files.append('video_predictions.csv')
