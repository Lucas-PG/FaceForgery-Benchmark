...
\usepackage{hyperref}
\hypersetup{hidelinks,pdftitle={Explaining Spatial and Spectral Face Forgery Detectors},pdfauthor={Research revision for author review}}
\title{Explaining Spatial and Spectral Face Forgery Detectors:\\Controlled Comparisons and Shared Failure Analysis}
\author{Lucas Cunha \quad Lucas Sotomaior \quad Lucas Gasperin\\Beatriz Caldas \quad Eduardo Pianovski \quad Rayson Laroca\\Pontifical Catholic University of Paran\'{a}, Curitiba, Brazil}
\iclrfinalcopy
\makeatletter
\def\iclr@conference{Pre-execution research revision, 11 September 2026; not submitted}
\makeatother
...
\paragraph{Frequency evidence and shortcut learning.}
Spectral discrepancies can expose regularities introduced by image generation \citep{frank2020}. Their presence does not imply robustness to acquisition changes or unseen generators. FreqDebias explicitly addresses frequency-dependent shortcuts using frequency diversification and consistency regularization \citep{kashiani2025}. This motivates a controlled comparison rather than an assumption that moving into frequency space reveals inherently superior forensic evidence. A frequency cluster could be informative, spurious, or partly caused by preprocessing.

\paragraph{Attribution methods have different semantics.}
Integrated Gradients (IG) measures a fixed output difference relative to a baseline along a path \citep{sundararajan2017}. Grad-CAM uses gradients to weight an intermediate activation map \citep{selvaraju2017}. LIME fits a local surrogate \citep{ribeiro2016}, while Kernel SHAP estimates feature-coalition contributions using a Shapley-oriented weighting scheme \citep{lundberg2017}. Attention rollout aggregates attention connectivity and is not equivalent to a class-specific gradient attribution \citep{abnar2020}. Comparing these methods is useful precisely because they answer different questions; their agreement is not a collection of independent confirmations.

\paragraph{Evaluation requires more than visual plausibility.}
Parameter-randomization tests can reveal explanations that are insensitive to learned parameters \citep{adebayo2018}. Perturbation-based evaluation can itself introduce distribution shift, motivating stronger controls such as remove-and-retrain protocols \citep{hooker2019}. Quantus organizes explanation evaluation around multiple properties rather than a single visual criterion \citep{hedstrom2023}. More recent work examines off-manifold paths \citep{zaher2024}, the consequences of grouping variables into coalitions \citep{zheng2025}, and formal certification rather than empirical smoothing \citep{anani2025}. FocusViT uses class-specific gradients and validation-based layer aggregation for transformer explanations \citep{ali2026}. These studies inform the present safeguards; manifold IG, certified explanations, FocusViT, and remove-and-retrain evaluation are not implemented or claimed as completed experiments here.

\section{Benchmark scope and provenance}
\subsection{Data and checkpoints}
The preceding study reports official MFFI partitions containing 524,429 training, 147,363 validation, and 181,947 test images, with a corresponding degraded test partition termed Test-Hard or Test-D \citep{cunha2026}. The proposed analysis reuses existing checkpoints and predictions and does not require retraining. Its primary roster is one RGB checkpoint, seed 42, from each of ResNet, Xception, MobileNet, ViT, CLIP, and DINO. An 18-checkpoint RGB roster using seeds 42, 123, and 2024 provides a separate shared-error sensitivity analysis. The roster is part of the saved experiment contract, not an implicit claim to cover every trained model.
...
Table~\ref{tab:legacy} preserves a compact set of results from the earlier paper \citep{cunha2026}. They are reported means from that study, not new runs of this extension or a claim about the supplied fine-tuned checkpoints. The table provides the empirical motivation for explanation experiments without filling their unmeasured outcomes with estimates.

\begin{table}[t]
\caption{Previously reported RGB ROC-AUC from \citet{cunha2026}. The final column is arithmetic subtraction of the reported rounded means. These are the earlier study's configurations, not a re-evaluation of the current released weights.}
\label{tab:legacy}
\centering
\begin{tabular}{lrrr}
\toprule
Earlier configuration & Clean & Test-Hard & Absolute drop\\
\midrule
Xception & 0.884 & 0.609 & 0.275\\
ResNet-18 & 0.846 & 0.635 & 0.211\\
MobileNetV3 & 0.839 & 0.594 & 0.245\\
ViT, from scratch & 0.624 & 0.608 & 0.016\\
CLIP-style encoder, from scratch & 0.750 & 0.619 & 0.131\\
DINOv3, frozen configuration & 0.809 & 0.726 & 0.083\\
\bottomrule
\end{tabular}
\end{table}
...
The earlier study also reports degraded AUC of 0.650 for Xception with RGB plus magnitude, compared with 0.609 for RGB; 0.648 for ResNet with a hybrid frequency stack, compared with 0.635 for RGB; and 0.612 for MobileNet with RGB plus magnitude, compared with 0.594 for RGB \citep{cunha2026}. These reported differences are consistent with potential complementarity. Without a controlled ablation and uncertainty analysis, however, they do not prove that spectral channels carry a particular manipulation artifact or that the benefit generalizes outside MFFI.

\subsection{Research questions for the new outputs}
\paragraph{Architectural evidence allocation.}
...
\paragraph{Complementary spectral evidence.}
...
\paragraph{Shared vulnerabilities.}
...
\appendix
\section{Artifact contract and execution defaults}
\label{app:reproduce}
The source baseline is \texttt{ICLR} commit \texttt{1643db7a82f111f1a468dba986fe29593761b6be}. Changes are isolated to \texttt{explicability} and proposed through a pull request into \texttt{ICLR}. The expected model source is \texttt{lucasoc/MFFI-Models}, revision \texttt{3bef179cdc08850e1d720182e55a040d00c9d582}. Local file paths alone do not authenticate that release: the plan records local artifact hashes and execution binds the exact checkpoint bytes.

The authoritative configuration is \texttt{configs/explicability.yaml}. Its primary roster contains the six RGB families at seed 42, with the declared fine-tuning regime; all three seeds are used for validation selection and a separate shared-error sensitivity analysis. Changing a regime, baseline, partition, roster, threshold rule, or estimator budget creates a new experiment contract rather than overwriting the previous one. The default encoded-zero baseline and one replicate are implementation defaults, not convergence or baseline-invariance findings.

\begin{table}[h]
\caption{Output responsibilities. These paths are generated by explicit commands; their existence is not claimed before execution.}
\centering\small
\begin{tabular}{p{.40\linewidth}p{.52\linewidth}}
\toprule
Artifact & Role\\
\midrule
\texttt{plan.json} & Immutable configuration, provenance, selection, expected predictions, and cohorts.\\
\texttt{prediction\_metrics.csv} & Recomputed metrics from supplied prediction exports, not newly run inference.\\
\texttt{task1/<model>/<method>/} & Common 64-image raw maps, metadata, and visualizations.\\
\texttt{task2/comparisons/} & Verified same-image RGB--frequency boards, per method.\\
\texttt{task3/}; \texttt{task3\_controls/} & Complete primary shared-error population and label-matched controls.\\
\texttt{coverage.json} & Complete, unsupported, missing, and corrupt cells, with control coverage.\\
\bottomrule
\end{tabular}
\end{table}

\section{Interpretation checklist for the executed revision}
Before replacing the pre-execution results boundary, report the exact source commit, checkpoint hashes, prediction exports, threshold policy, and cohort hash. Recompute numerical tables from the saved files; do not transfer scratch-model values into a fine-tuned table. Record every requested method's coverage and numerical diagnostics, including constant maps and unsupported combinations. Keep raw signed values separate from independently normalized displays. Report selected examples with their reference stratum and each model's own prediction, and make the complete gallery available for review.

For Task 2, identify the validation-selected architecture pair and exclusions, label frequency axes, and avoid cross-domain pixel-localization claims. For Task 3, report the roster-specific denominator and all shared errors before discussing nuisance conditions. Include matched controls, annotation uncertainty, and alternative explanations. Replace hypotheses with observed conclusions only when the corresponding outputs support them; a null result or disagreement between methods is a scientific result rather than an execution defect.
\end{document}
