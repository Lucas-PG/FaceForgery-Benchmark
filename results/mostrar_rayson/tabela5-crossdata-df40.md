...
| 3 | **CLIP ViT-B/16** | `concat` | `finetune` | 5-6x | **0.7650 ± 0.0323** | 71.17% ± 2.66% | 74.30% ± 2.76% | 72.99% ± 1.92% | 75.72% ± 4.18% | 65.57% ± 2.68% |
| 4 | **CLIP ViT-B/16** | `none` | `finetune` | 5-6x | **0.7639 ± 0.0460** | 72.18% ± 3.23% | 75.85% ± 2.45% | 72.93% ± 3.58% | 79.16% ± 3.03% | 63.60% ± 7.04% |
| 5 | **DINO (ConvNeXt-B)** | `concat` | `finetune` | 5-6x | **0.7558 ± 0.0207** | 69.81% ± 1.78% | 73.72% ± 1.56% | 70.91% ± 1.73% | 76.80% ± 2.28% | 61.22% ± 3.25% |
| 6 | **DINO (ConvNeXt-B)** | `none` | `finetune_robust` | 2x | **0.7517** | 68.21% | 73.19% | - | - | - |
| 7 | **Xception** | `concat` | `finetune` | 5-6x | **0.7344 ± 0.0108** | 68.11% ± 0.42% | 74.02% ± 0.32% | 67.20% ± 0.44% | 82.38% ± 0.70% | 50.56% ± 1.20% |
| 8 | **MobileNetV3-Large** | `none` | `finetune_robust` | 1x | **0.7304** | 68.91% | 71.20% | - | - | - |
| 9 | **Xception** | `none` | `finetune` | 5-6x | **0.7298 ± 0.0244** | 69.28% ± 1.08% | 75.04% ± 0.35% | 68.05% ± 1.44% | 83.69% ± 1.48% | 51.57% ± 4.18% |
| 10 | **Vision Transformer (ViT-B/16)** | `none` | `finetune_robust` | 5x | **0.7283 ± 0.0152** | 68.99% ± 1.13% | 72.29% ± 0.93% | - | - | - |
| 11 | **DINO (ConvNeXt-B)** | `none` | `finetune` | 5-6x | **0.7208 ± 0.0289** | 65.90% ± 2.13% | 70.40% ± 2.26% | 67.48% ± 1.57% | 73.65% ± 3.73% | 56.36% ± 2.60% |
| 12 | **ResNet-18** | `concat_frequency` | `finetune` | 5-6x | **0.7071 ± 0.0316** | 63.83% ± 2.68% | 71.41% ± 1.96% | 63.33% ± 2.13% | 81.87% ± 2.32% | 41.64% ± 4.64% |
...
| 17 | **Xception** | `none` | `finetune_robust` | 1x | **0.6814** | 67.25% | 74.43% | - | - | - |
| 18 | **MobileNetV3-Large** | `concat` | `finetune` | 5-6x | **0.6676 ± 0.0177** | 63.13% ± 1.31% | 63.59% ± 1.33% | 69.92% ± 2.36% | 58.41% ± 2.65% | 68.94% ± 4.33% |
| 19 | **ResNet-18** | `none` | `finetune_robust` | 1x | **0.6451** | 62.75% | 66.42% | - | - | - |
